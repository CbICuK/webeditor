$(document).ready(function () {
    function validatePassword() {
        ...
        if (password.length > 0 && !isPasswordValid) {
            ...
            $('#new_password').removeClass('valid invalid');
        }
        ...
        resetButton.prop('disabled', !(isPasswordValid && password === confirmPassword));
    }
    ...
    $('#resetButton').on('submit', function (event) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    $('#messageBox').text(response.responseJSON.message).css('color', 'green');
                } else {
    ...
    $('#new_password, #confirm_password').on('input', validatePassword);
});